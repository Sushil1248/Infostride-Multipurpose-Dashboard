...
const canEditPermission = async (req, domainHeader) => {
    try {
        ...
        if (userPermittedWebsite.includes(domainHeader)) {
            return user.permissions[domainHeader].editor_permission;
        }
...
const getPostById = async (req, res) => {
    try {
        ...
        if (featuredImageId && mongoose.Types.ObjectId.isValid(featuredImageId)) {
            ...
            media.id = media._id;
            delete media._id;
            const updatedPost = { ...post.toObject(), id: post._id, featuredImage: media, categoryObject: categoryObject, can_edit };
            ResponseHandler.success(res, { post: updatedPost }, 200);
        } else {
            const updatedPost = { ...post.toObject(), id: post._id, categoryObject: categoryObject, can_edit };
            ResponseHandler.success(res, { post: updatedPost }, 200);
        }
    } catch (error) {
        ErrorHandler.handleError(error, res);
...
const getAllPosts = async (req, res) => {
    try {
        ...
        if (user?.role?.name !== 'admin') {
            if(userPermittedWebsite.includes(domainHeader)){
                can_edit = user.permissions[domainHeader].editor_permission;
            }
        }
        const { post_type } = req.params;
        const query = {};
        console.log(domainHeader, userPermittedWebsite, user.permissions)

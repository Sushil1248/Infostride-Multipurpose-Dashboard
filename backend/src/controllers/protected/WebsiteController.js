...
const createUnderScoreSlug = (inputString) => {
...
const createOrEditWebsite = async (req, res) => {
    try {
        ...
        const { icon, business_name, url, description, id, menus } = req.body.website; // Extract website data from request body
...
const listWebsites = async (req, res) => {
    try {
        const websites = await Website.find({}, 'icon business_name description url'); // Limit fields to be retrieved
...
const listWebsitesWithMenus = async (req, res) => {
    try {
        ...
        if (user?.role?.name !== 'admin') {
            const filteredWebsites = websitesData?.filter((website) => userPermittedWebsite.includes(createUnderScoreSlug(website.business_name)));
...
const deleteWebsite = async (req, res) => {
    try {
        ...
        const deletedWebsite = await Website.findByIdAndDelete(website_id);
        if (!deletedWebsite) {
            throw new CustomError(404, 'Website not found');
        }

        // Respond with success message
        ResponseHandler.success(res, { message: 'Website deleted successfully' }, 200);
    } catch (error) {
        ErrorHandler.handleError(error, res);
    }
};
